...
class ModelosViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let controller = segue.destination as? DetalleModeloViewController {
            controller.objModelo = sender as? Modelo
        }
    }
}
...
extension ModelosViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let objModelo = self.arrayModelos[indexPath.row]
        self.performSegue(withIdentifier: "DetalleModeloViewController", sender: objModelo)
    }
