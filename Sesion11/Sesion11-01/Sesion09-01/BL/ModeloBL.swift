...
class ModeloBL {
    ...
    class func crear(fabricacion: String?, nombre: String?, tipoCarroceria: String?, urlImagen: String?, success: AgregarModelo, mensajeError: MensajeError) {
        ...
        guard let urlImagenSegura = urlImagen, urlImagenSegura.count != 0 else {
            mensajeError("Ingresa una url correcta")
            return
        }
        ...
        let objModelo = ModeloDA.crear(fabricacion: fabricacionSeguro, nombre: nombreSeguro, tipoCarroceria: tipoCarroceriaSeguro, urlImagen: urlImagenSegura, contexto: contexto)
